...
import { formatDurationByLocale, formatNumber } from "../../../../../helpers";
...
const PlaceBidModal = ({ show, onClose, data, user, auction_id }) => {
    ...
    const fallbackBid = [submittedBid, reg_bid_amount].find(
        (val) => val != null && val !== "" && !isNaN(Number(val))
    );
    const fallbackAutoBid = [submittedBid, my_max_bid_val, reg_bid_amount].find(
        (val) => val != null && val !== "" && !isNaN(Number(val))
    );
